 ...
 * Planning实现中，逻辑计算的基础数据结构，很多操作都会在这个数据结构上进行（例如：交通规则逻辑，障碍物投影，路径优化，速度决策等）。
...
class ReferenceLineInfo {
 public:
  ...
  bool AdjustTrajectoryWhichStartsFromCurrentPos(const common::TrajectoryPoint& planning_start_point,
  ...
  hdmap::PathOverlap* GetOverlapOnReferenceLine(const std::string& overlap_id, 
 ...
 private:
  ...
  /**
   * @brief Data generated by speed_bounds_decider for constructing st_graph for different st optimizer
   */
