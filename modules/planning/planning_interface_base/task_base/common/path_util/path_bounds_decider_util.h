...
class PathBoundsDeciderUtil {
 public:
   ...
   */
  static bool GetBoundaryFromLanesAndADC(const ReferenceLineInfo& reference_line_info,
   ...
   */
  static bool UpdatePathBoundaryWithBuffer(double left_bound, double right_bound, 
   ...
   */
  static bool UpdateRightPathBoundaryWithBuffer(double right_bound, BoundType right_type, 
                                                std::string right_id,
                                                PathBoundPoint* const bound_point);
  static bool UpdateLeftPathBoundaryWithBuffer(double left_bound, BoundType left_type, 
                                               std::string left_id,
                                               PathBoundPoint* const bound_point);
  ...
  static bool GetBoundaryFromStaticObstacles(const ReferenceLineInfo& reference_line_info,
  ...
  static bool GetBoundaryFromStaticObstaclesWithNudge(const ReferenceLineInfo& reference_line_info,
                                                      const SLState& init_sl_state, PathBoundary* const path_boundaries,
                                                      std::string* const blocking_obstacle_id, double* const narrowest_width,
                                                      LaneBorrowInfo lane_borrow_info = LaneBorrowInfo::NO_BORROW);

  static std::vector<ObstacleEdge> SortObstaclesForSweepLine(const IndexedList<std::string, Obstacle>& indexed_obstacles,
                                                             const SLState& init_sl_state);
   ...
   *  @param left_bound minimum left boundary (l_max)
   *  @param right_bound maximum right boundary (l_min)
   *  @param path_boundaries path_boundaries (its content at idx will be updated)
   *  @param center_line center_line (to be updated)
   *  @return If path is good, true; if path is blocked, false.
   */
  static bool UpdatePathBoundaryAndCenterLineWithBuffer(size_t idx, double left_bound, double right_bound,
   ...
   */
  static bool GetBoundaryFromSelfLane(const ReferenceLineInfo& reference_line_info,
  ...
  static bool ExtendBoundaryByADC(const ReferenceLineInfo& reference_line_info,
                                  ...
                                  PathBoundary* const path_bound);
  static void ConvertBoundarySAxisFromLaneCenterToRefLine(const ReferenceLineInfo& reference_line_info,
                                                          PathBoundary* const path_bound);
  static int IsPointWithinPathBound(const ReferenceLineInfo& reference_line_info, const double x,
                                    const double y, const PathBound& path_bound);
  static void GetSLPolygons(const IndexedObstacles& obstacles,
                            std::vector<SLPolygon>* polygons,
                            const SLState& init_sl_state);
  static void GetSLPolygonsWithNudgeDecision(const ReferenceLineInfo& reference_line_info,
                                             const IndexedObstacles& obstacles, std::vector<SLPolygon>* polygons,
                                             const SLState& init_sl_state, LaneBorrowInfo lane_borrow_info);
