...
void TrafficLight::MakeDecisions(Frame* const frame,
  ...
  // debug info
  planning_internal::SignalLightDebug* signal_light_debug = reference_line_info->mutable_debug()
  ...
  for (const auto& traffic_light_overlap : traffic_light_overlaps) {
    ...
    const double stop_deceleration = util::GetADCStopDeceleration(injector_->vehicle_state(), adc_front_edge_s,
